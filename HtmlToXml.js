...
HtmlAsXml = function () {
    return {
            ...
            for (var i = 0; i < elem.attributes.length; i++) {
                ...
                if (attrib.specified) {
                    ...
                    if (attrib.value) {
                        ...
                        //returnString += attrib.name + "=\"" + (this._stringContains(attrib.name, "data-") ? this._parseDataTagAttribute(attrib) : this._htmlEncode(attrib.value)) + "\" ";
                        returnString += attrib.name + "=\"" + this._parseElementAttributeValue(attrib) + "\" ";
                    }
                }
            }
            return returnString;
        ...
        _parseElementAttributeValue: function (attrib) {
            return this._escapeJavascriptNuances(this._htmlEncode(attrib.value));
        },

        //used to escape the common js nuances of single quotes, double quotes, and the backslash
        _escapeJavascriptNuances: function (value) {
        ...
        _htmlEncode: function (value) {
            ...
            if (this._stringContains(value, "\\")) {
                value = this._escapeJavascriptNuances(value);
            }
            return value;
        },
        ...
        _recurseIFrameElement: function (elem) {
            ...
            for (var i = 0; i < elem.attributes.length; i++) {
                ...
                if (attrib.specified) {
                    ...
                    if (attrib.name === 'id' || attrib.name === 'class') {
                        jQueryLookup += "[" + attrib.name + "='" + this._htmlEncode(attrib.value) + "']";
                    }
                }
            }
            jQueryLookup += "\")";
            ...
            if (domElem) {
                ...
                if (domElem.contents) {
                    if (domElem.contents().length === 0) {
                        ...
                        return this._recurseElementFix(domElem.contents().find('*')[0]);
                    }
                } else if (domElem.contentDocument) {
                    return "<body>" + this._recurseElementFix(domElem.contentDocument.documentElement.getElementsByTagName("body")[0]) + "</body>";
                } else {
        ...
        _recurseElementFix: function (elem) {
            try {
                var tagName = elem.tagName.toLowerCase();
                var returnString = "<" + tagName + this._getElementAttributes(elem) + ">";
                ...
                if (tagName !== "iframe") {
                    ...
                    returnString += this._tryGetInnerText(elem);

                    var topLevelElems = elem.children;
                    for (var i = 0; i < topLevelElems.length; i++) {
                        returnString += this._recurseElementFix(topLevelElems[i]);
                    }
                } else {
                    ...
                    returnString += this._recurseIFrameElement(elem);
                }
            }
            catch (exp) {
                //eating this atm
        ...
        _fixFormatting: function () {
            ...
            if (parserError) {
                ...
                body = new DOMParser().parseFromString(this._myHtml, "text/html");
                //attempt to recurse the html string into a valid xml-format string
                htmlDoc = this._recurseElementFix(body.childNodes[1]);
            }
        ...
        _stringContains: function (str, contains) {
        ...
        _tryGetInnerText: function (elem) {
            var text = null;
            try {
                ...
                text = this._htmlEncode(window.$(elem).contents().filter(function () { return this.nodeType == 3; }).text());
            } catch (exp) {
                ...
                //return "<innerText>" + this._htmlEncode(text.trim().replace(/[\n\r\t]/g, '')) + "</innerText>";
                return (text.length > 0 ? "<innerText>" + this._htmlEncode(text.replace(/[\n\r\t]/g, '')) + "</innerText>" : "");
            }
        },

        //converts the html of the current page into an xml document
        toXml: function () {
            return this._fixFormatting();
        },
